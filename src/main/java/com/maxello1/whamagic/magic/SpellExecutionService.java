...
public final class SpellExecutionService {
    ...
    public static void execute(Level level, Player player, SpellIr spell) {
        ...
        ElementType primaryElement = elements.isEmpty() ? null : elements.get(0);
    ...
    private static void executeWind(ServerLevel level, Player player, double power, ManifestationModifier modifier) {
        ...
        for (LivingEntity e : entities) {
            if (modifier == ManifestationModifier.LEVITATION) {
                ...
                Vec3 dir = new Vec3(
                        targetPos.getX() + 0.5 - e.getX(),
                        targetPos.getY() + 0.5 - e.getY(),
                        targetPos.getZ() + 0.5 - e.getZ()).normalize();
                e.setDeltaMovement(e.getDeltaMovement().add(dir.scale(power * 0.5)));
            } else {
                Vec3 dir = new Vec3(
                        e.getX() - player.getX(),
                        e.getY() - player.getY(),
                        e.getZ() - player.getZ()).normalize();
        ...
        for (int x = -size; x <= size; x++) {
            for (int z = -size; z <= size; z++) {
                ...
                if (state.isAir() || state.canBeReplaced()) {
                    ...
                    // Non-permanent earth magic remains visual-only.
                    level.sendParticles(
                ...
                if (modifier == ManifestationModifier.COLUMN && blocksChanged < maxBlocks) {
                    for (int y = 1; y <= size * 2; y++) {
                        ...
                        BlockState columnState = level.getBlockState(py);
